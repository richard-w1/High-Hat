...
async function refreshThreats() {
  try {
    console.log('🔍 Loading threats from API...');
    ...
    console.log('📥 Threats API Response:', data);
    ...
    console.log(`✅ Loaded ${allThreats.length} threats`);
...
function updateStatistics() {
  ...
  const avgConf = allThreats.length > 0 ? 
    allThreats.reduce((sum, t) => sum + (t.threat_confidence || 0), 0) / allThreats.length : 0;
...
function filterThreats() {
  ...
  filteredThreats = allThreats.filter(threat => {
    ...
    if (confidenceFilter !== 'all') {
      const conf = (threat.threat_confidence || 0);
...
function renderThreats() {
  ...
  container.innerHTML = filteredThreats.map(threat => {
    ...
    const confidence = (threat.threat_confidence || 0);
...
// Audio Alert System (same as dashboard)
  ...
  try {
    ...
    if (data.has_alert && data.alert) {
      ...
      if (alert.timestamp !== lastAlertTimestamp) {
        ...
        // Refresh threats list to show new threat
        refreshThreats();
      }
    }
  } catch (error) {
...
function showVisualAlert(alert) {
  ...
  if (!document.getElementById('alertAnimations')) {
    ...
    style.textContent = `
      ...
      @keyframes alertShake { 0%, 100% { transform: rotate(0deg); } 25% { transform: rotate(-10deg); } 75% { transform: rotate(10deg); } }
    `;
